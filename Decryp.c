...
int main(int argc, char **argv) {//** for string array and * is pointed to the character string.
    ...
    fread(nonce, 1, 8, stdin);//output to destination
    ...
    while (!done) {//
        ...
        chacha20_decrypt(&chacha, plain, crypt, len_read);//decrypt the plain into crypt through chacha with the length read
